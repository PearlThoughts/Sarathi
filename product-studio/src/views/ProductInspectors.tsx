...
export const CompactInspector = ({
  ...
  if (relation !== undefined) {
    ...
    return (
      <article
        ...
        className="pointer-events-auto w-[min(27rem,calc(100vw-2rem))] rounded-2xl border border-teal-800/70 bg-stone-950/95 p-4 shadow-2xl backdrop-blur"
      >
  ...
  const childCount = map.entities.filter(({ parentId }) => parentId === selected.entityId).length;
  const relationCount = map.relations.filter(({ source, target }) =>
    [source, target].some(
  ...
  return (
    <article
      ...
      className="pointer-events-auto w-[min(25rem,calc(100vw-2rem))] rounded-2xl border border-stone-800 bg-stone-950/95 p-4 shadow-2xl backdrop-blur"
    >
      ...
      <p className="mt-3 line-clamp-3 text-pretty text-sm leading-relaxed text-stone-300">
      ...
      <div className="mt-4 flex flex-wrap gap-3 text-xs text-stone-400">
        <span>{childCount} immediate children</span>
        <span>{relationCount} relationships</span>
        <span className="capitalize">{selected.lifecycle}</span>
      </div>
      {(dossier?.safeWarnings.length ?? 0) > 0 ? (
        <p className="mt-3 border-l-2 border-amber-300 pl-3 text-xs text-amber-100">
          {dossier?.safeWarnings[0]}
        </p>
      ) : null}
      <div className="mt-4 flex flex-wrap gap-2">
        <button
          className="rounded-lg bg-teal-300 px-3 py-2 text-xs font-semibold text-stone-950 hover:bg-teal-200 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white"
...
export const FullDossier = ({
  ...
  return (
    <div
      ...
      role="dialog"
    >
      <section className="h-full w-[min(58rem,calc(100vw-1rem))] min-w-80 resize-x overflow-y-auto border-l border-stone-700 bg-stone-950 p-5 text-stone-100 shadow-2xl sm:p-7">
                  ...
                  {history.events.map((event) => (
                    <li className="relative pb-6" key={event.id}>
                      ...
                      <p className="mt-1 text-xs text-stone-400">
                        Revision {event.revision} · valid{" "}
                        {new Date(event.validFrom).toLocaleString()}
                      </p>
                      <button
