...
The versioned evaluation runner must execute the same authorized answer path as production. Its stored output is privacy-safe aggregate evidence: case IDs, failure codes, answer fingerprints, acceptance dimensions, citation counts, authorization checks, and human ratings. Real project questions, expected terms, and ratings remain in the private deployment overlay. Expected denials and fail-closed empty answers are scored separately from answer-quality dimensions. A human rating is valid only for the exact reviewed answer fingerprint; declared human-usefulness thresholds fail closed when a quality answer is unrated or changes after review.
