...
- Command: `bun run test`
- Location: `tests/knowledge-*.test.ts`, `tests/jira-knowledge-source.test.ts`, `tests/vault-knowledge-source.test.ts`, `tests/github-knowledge-*.test.ts`, `tests/teams-knowledge-source.test.ts`, `tests/teams-change-notification-subscription.test.ts`
- Scope: Canonical Jira/Vault/GitHub/Teams ingestion and delivery projections; source-qualified canonical entity aliases and explicit source-created/source-updated/observed/indexed time semantics; Jira overlap cursors, project/field/board/sprint catalog, changelog, and status-wait intervals; immutable Vault tree/blob deltas, rename/deletion retirement, and changed-only fetches; exact-revision GitHub default-branch bootstrap, symbol and line passages, code exclusions, activity observations, and changed-only repair; bounded Teams channel/thread history, contextual reply passages, edits, deletions, missed-event repair, privacy exclusions, trusted pagination, and change-notification renewal; versioned attributed assertions with stable authors, aliases, confidence, effective dates, corrections, and supersession; continuous-sync control contracts and leased backfill/event/hourly orchestration; AI SDK and deterministic embeddings; ACL-first exact/full-text/vector retrieval; live GitHub search; RRF fusion; concise cited answer validation; Teams composition; CLI; and privacy-safe failures.
- Prerequisites: Bun dependencies installed with `bun install`.
...
- Command: `bunx vitest run tests/product-model-api.test.ts`
- Location: `tests/product-model-api.test.ts`
