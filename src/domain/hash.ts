const sha256InitialHash: readonly number[] = [
...
export const stableSha256 = (input: string): string => {
  ...
  for (let offset = 0; offset < padded.length; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temp1 =
        ((h ?? 0) + sum1 + choice + (sha256RoundConstants[index] ?? 0) + (words[index] ?? 0)) >>> 0;
...
export const uniqueId = (prefix: string): string => `${prefix}:${globalThis.crypto.randomUUID()}`;

const padSha256 = (bytes: Uint8Array): Uint8Array => {
  const lengthWithOneAndLength = bytes.length + 1 + 8;
  const paddedLength = Math.ceil(lengthWithOneAndLength / 64) * 64;
